...
function biaoge(){
	...
	if(data==0){
    	...
    	$.each(data, function(i,item) {
    		...
    		v1+=parseInt(item.vg8);
    		v2+=parseInt(item.vg81);
    		xu=i+1;
    		html+='<tr><td class="text-center">'+xu+'</td><td class="text-center">'+item.name+'</td>'+
    		'<td class="text-center">'+item.vg8+'</td>'+
    		'<td class="text-center">'+item.vg81+'</td>'+
    		'<td class="text-center">'+item.b8+'%</td></tr>';
    	});
    	...
    	html1='解决通电和农网改造:解决通电和农网改造'+v1+'户，其中贫困'+v2+'户'
		$("#biaozhu").html(html1);
    	//对下方的表格赋值
...
function pkcsbbz(){
	...
	$.each(data,function(i,item){
		count[i]=item.name;
		count2[i]=parseInt(item.vg8)-parseInt(item.vg81);
		count3[i]=item.vg81;
	});
