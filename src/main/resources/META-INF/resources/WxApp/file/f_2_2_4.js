...
function biaoge(){
	...
	if(data==0){
    	...
    	$.each(data, function(i,item) {
    		...
    		v1+=parseInt(item.vg9);
    		v2+=parseInt(item.vg91);
    		xu=i+1;
    		html+='<tr><td class="text-center">'+xu+'</td><td class="text-center">'+item.name+'</td>'+
    		'<td class="text-center">'+item.vg9+'</td>'+
    		'<td class="text-center">'+item.vg91+'</td>'+
...
function pkcsbbz(){
	...
	$.each(data,function(i,item){
		count[i]=item.name;
		count2[i]=parseInt(item.vg9)-parseInt(item.vg91);
		count3[i]=item.vg91;
	});
