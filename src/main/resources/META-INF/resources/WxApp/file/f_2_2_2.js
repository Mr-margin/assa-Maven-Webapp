...
function biaoge(){
	...
	if(data==0){
    	...
    	$.each(data, function(i,item) {
    		...
    		v1+=parseInt(item.vg7);
    		v2+=parseInt(item.vg71);
    		xu=i+1;
    		html+='<tr><td class="text-center">'+xu+'</td><td class="text-center">'+item.name+'</td>'+
    		'<td class="text-center">'+item.vg7+'</td>'+
    		'<td class="text-center">'+item.vg71+'</td>'+
...
function pkcsbbz(){
	...
	$.each(data,function(i,item){
		count[i]=item.name;
		count2[i]=parseInt(item.vg7)-parseInt(item.vg71);
		count3[i]=item.vg71;
	});
