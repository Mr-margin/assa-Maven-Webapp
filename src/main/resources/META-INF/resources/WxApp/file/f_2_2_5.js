...
function biaoge(){
	...
	if(data==0){
    	...
    	$.each(data, function(i,item) {
    		...
    		v1+=parseInt(item.vg14);
    		v2+=parseInt(item.vg141);
    		xu=i+1;
    		html+='<tr><td class="text-center">'+xu+'</td><td class="text-center">'+item.name+'</td>'+
    		'<td class="text-center">'+item.vg14+'</td>'+
    		'<td class="text-center">'+item.vg141+'</td>'+
    		'<td class="text-center">'+item.b14+'%</td></tr>';
    	});
    	...
    	html1='新增养老概况:新增养老'+v1+'人，其中贫困'+v2+'人'
		$("#biaozhu").html(html1);
    	//对下方的表格赋值
...
function pkcsbbz(){
	...
	$.each(data,function(i,item){
		count[i]=item.name;
		count2[i]=parseInt(item.vg14)-parseInt(item.vg141);
		count3[i]=item.vg141;
	});
