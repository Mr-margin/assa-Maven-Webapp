...
function biaoge(){
	...
	if(data==0){
    	...
    	$.each(data, function(i,item) {
    		...
    		v1+=parseInt(item.vg16);
    		v2+=parseInt(item.vg161);
    		xu=i+1;
    		html+='<tr><td class="text-center">'+xu+'</td><td class="text-center">'+item.name+'</td>'+
    		'<td class="text-center">'+item.vg16+'</td>'+
    		'<td class="text-center">'+item.vg161+'</td>'+
    		'<td class="text-center">'+item.b16+'%</td></tr>';
    	});
    	...
    	html1='常住人口社会保障任务:新增低保'+v1+'人，其中贫困'+v2+'人'
		$("#biaozhu").html(html1);
    	//对下方的表格赋值
...
function pkcsbbz(){
	...
	$.each(data,function(i,item){
		count[i]=item.name;
		count2[i]=parseInt(item.vg16)-parseInt(item.vg161);
		count3[i]=item.vg161;
	});
