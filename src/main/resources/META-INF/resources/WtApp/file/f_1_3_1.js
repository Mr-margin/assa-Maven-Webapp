...
//	myChart_ring = echarts.init(document.getElementById('tu_3'));//声明id为myChart_ring的div为图形dom
	var data = JSON.parse(ajax_async_t("../../getProjectBenefitHousehold.do", {name:obj.com},"text","1")); //调用ajax通用方法
	if(data==0){
...
//	tables +='<tr><td class="text-center">'+(i+1)+'</td><td class="text-center">'+ item.V1 +'</td>'+
	var data = ajax_async_t("/assa/getProjectBenefitHousehold.do",{name:obj.com,desc:"asc"},"json");
	var html = "";
	if(data=='' || data == null || data==undefined){
		...
		$.each(data,function(i,item){
				...
				'<td class="text-center">'+item.count_num+'</td>'+
				'</tr>';
				if(item.count_num == '' || item.count_num == null || item.count_num == undefined){
					
				}else{
					A1=parseInt(item.count_num)+parseInt(A1);
