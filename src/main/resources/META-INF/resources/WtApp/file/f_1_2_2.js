...
//	myChart_ring = echarts.init(document.getElementById('tu_3'));//声明id为myChart_ring的div为图形dom
	var data = JSON.parse(ajax_async_t("../../getProjectBonusPracticable.do", {name:obj.com},"text","1")); //调用ajax通用方法
	if(data==0){
		...
		$.each(data,function(i,item){
			...
			if(item.practicable_capital == '' || item.practicable_capital == null || item.practicable_capital == undefined){
				
			}else{
				count_2[i]=parseInt(item.practicable_capital);
...
//	tables +='<tr><td class="text-center">'+(i+1)+'</td><td class="text-center">'+ item.V1 +'</td>'+
	var data = ajax_async_t("/assa/getProjectBonusPracticable.do",{name:obj.com,desc:"asc"},"json");
	var html = "";
	if(data=='' || data == null || data==undefined){
		...
		$.each(data,function(i,item){
				...
				'<td class="text-center">'+item.practicable_capital+'</td>'+
				'</tr>';
				if(item.practicable_capital == '' || item.practicable_capital == null || item.practicable_capital == undefined){
					
				}else{
					A1=parseInt(item.practicable_capital)+parseInt(A1);
